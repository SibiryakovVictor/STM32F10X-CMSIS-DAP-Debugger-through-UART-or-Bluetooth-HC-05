...
extern uint8_t  SWD_Transfer    (uint32_t request, uint32_t *data); //МЕНЯЛОСЬ НАЗВАНИЕ ДЛЯ ПРОВЕРКИ

extern uint32_t DAP_ProcessVendorCommand (uint8_t *request, uint8_t *response);

uint32_t DAP_ProcessCommandNew (uint8_t *request, uint8_t *response); //МЕНЯЛОСЬ НАЗВАНИЕ ДЛЯ ПРОВЕРКИ
