...
uint32_t DAP_ProcessCommand(uint8_t *request, uint8_t * response)
{
    ...
    //важно запомнить именно несмещенный указатель на буффер для ответа (response),
    //чтобы в отправляемый хосту пакет вложился и заголовок команды;
    uint8_t * pointerResponse = response;
    *response++ = *request;
    ...
    case ID_DAP_Transfer:
        ...
        HAL_UART_Transmit( &huart1, pointerResponse, DAP_PACKET_SIZE, 255 );
        break;
